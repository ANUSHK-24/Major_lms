...
setFilterCourses(courseData)
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      {/* Sidebar */}
      <aside className={`w-65 h-screen overflow-y-auto bg-black fixed  top-0 left-0 p-6 py-32.5 border-r border-gray-200 shadow-md transition-transform duration-300 z-5 
        ...
        <form className="space-y-4 text-sm  bg-gray-600 border-white text-[white] border  p-5 rounded-2xl" onSubmit={(e)=>e.preventDefault()}>
          <button className='px-2.5 py-2.5  bg-black text-white  rounded-[10px] text-[15px] font-light flex items-center justify-center gap-2 cursor-pointer' onClick={()=>navigate("/searchwithai")}>Search with AI <img src={ai} className='w-7.5 h-7.5 rounded-full' alt="" /></button>
